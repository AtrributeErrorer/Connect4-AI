...
<img src="https://github.com/AtrributeErrorer/Connect4-AI/blob/750ed0872573640263ead42f3f8fc3b3f7e3ef26/pygame%20window.jpg" width="500">
...
Depth =      (VALUES: Any positive number, DEFAULT: 10) How deep the search should be,
                      lower is faster but less intelligent, higher is smarter but more         
                      computationally expensive
```

## How it works:
If Player_X in config.ini is set to a value of 1, then a human will make a move,
- To make a move as a human, you simply click on the column you want to play in

If Player_X in config.ini is set to a value of 2, then the AI will make a move,
- The AI emulates future moves by using the **negamax algorithm** (a variation of the *Minimax algorithm*) 
- AI looks for *depth* moves ahead emulating opponent responses and AI responses, the *fastest* win gets the highest score possible and  slower wins get less score
- The AI *prunes* bad plays it would never pick to reduce computation time using **Alpha-Beta pruning**
- Once the function finishes evaluating a board state, it saves information of the node to a **Zobrist transposition table**
  to avoid re-evaluating the same board state in the future
...
## Installation:
Download the respository and make sure you have Python 3.14 installed
Download pygame-ce by typing `pip install pygame-ce` in cmd
